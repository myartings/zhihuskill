...
大部分命令无需登录（已内置多通道降级：v4 API → 移动端 API → 页面 HTML）。
回答全文和文章全文在无 Cookie 时可能为截断版，导入 Cookie 后可读完整内容。
...
导入浏览器 Cookie 后可读取回答/文章完整正文（关键 Cookie: `z_c0`）。

Cookie 查找顺序（先找到的优先）：
1. 环境变量 `$ZHIHU_COOKIE_FILE`
2. **`~/.config/zhihu/cookies.json`**（共享路径，推荐，所有 Agent 共用）
3. `<SKILL_DIR>/cookies.json`（向后兼容）

导入方法：
1. 用浏览器登录 zhihu.com
2. 安装 Cookie-Editor 扩展，导出 JSON
3. 保存到 `~/.config/zhihu/cookies.json`
